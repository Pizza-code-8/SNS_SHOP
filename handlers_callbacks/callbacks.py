...
async def ref_list(callback: CallbackQuery, state: FSMContext):
    ...
    if user.status == "left":
        ...
        await callback.message.edit_text(
            f"💸Условия нашей реферальной системы\n\n💵<b>5%</b> - за 5 приглашенных друзей\n💰<b>10%</b> - за 25 приглашенных друзей\n💎<b>20%</b> - за 100 приглашенных друзей\n\n❗️После того, как друг прийдет по ссылке, ему нужно будет сделать один заказ, для успешной активации скидки\n\n🔥<b>Скидка действует пожизненно на все заказы</b>🔥\n\n🗣Скорей начинай делиться своей рефералкой: <code>{take_ref_link(callback.from_user.id)}</code>",
            parse_mode=ParseMode.HTML,
...
async def address_order(message: Message, state: FSMContext):
    ...
    add_user_id_orders(user_id=uid, order_id=id, time = now)
    add_username_in_orders(user_id=uid, u_name=username)
